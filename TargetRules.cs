...
namespace UnrealBuildTool
{
	...
	public enum TargetBuildEnvironment
	{
		...
		/// 引擎二进制文件和中间文件输出到引擎文件夹。对引擎构建环境的特定于目标的修改将被忽略。
		/// </summary>
		Shared,

		/// <summary>
		/// Engine binaries and intermediates are specific to this target
		/// 引擎二进制文件和中间文件是特定于此目标的
		/// </summary>
		Unique,
	}
	...
	public abstract partial class TargetRules
	{
		...
		/// <summary>
		/// The current build version  当前构建的版本
		/// </summary>
		...
		/// 是否使用CEF3 (Chromium Embedded Framework)
		/// </summary>
		...
		/// 项目是否使用可视化的Slate UI(相对于总是可用的低级窗口/消息传递)。
		/// </summary>
		...
		/// 强制链接到静态CRT上。这在整个引擎中并不完全受支持，因为需要共享(例如)分配器实现，并且TPS库彼此保持一致，但可以用于实用程序。
		/// </summary>
		...
		/// 这个目标的输出是否可以公开分发，即使它依赖于具有特殊限制的文件夹中的模块(例如。Carefullyredist, NotForLicensees, NoRedist)。
		/// </summary>
		[XmlConfigFile(Category = "BuildConfiguration")]
		...
		/// 构建所有模块，哪些值是目标类型的。使用CIS和创建安装引擎构建
		/// </summary>
		...
		/// 使用BuildPlugins来配置一个列表，这个列表存的是编译插件的列表。
		/// </summary>
		...
		/// in ModuleHostType.
		/// 一个附加插件的列表，它需要被添加到目标程序，它允许引用非选项的插件模块，那些不能被禁用的，并且允许构建指定的模块在程序目标中.
		/// </summary>
		...
		/// 禁用的插件列表，提示：这个项目可能继续引用他们，因此他们需要表姐选项，避免失败的查找他们在运行时态。
		/// </summary>
		...
		/// 是否将目标构建成一个dll，请求linkType被设置为 TargetLinkType.Monolithic.
		/// </summary>
		[RequiresUniqueBuildEnvironment]
		...
		/// 场景查询加速是否由UE4完成。physx场景查询结构仍然被创建，但是我们没有使用它。
		/// </summary>
		[RequiresUniqueBuildEnvironment]
		...
		/// 是否禁用WITH_EDITORONLY_DATA编译。只有Windows会使用它，其他平台会强制将其设为false。
		/// </summary>
		[RequiresUniqueBuildEnvironment]
		...
		/// 我们是否应该使用自定义的“虚幻”平台(true)编译SQLite，还是使用本机平台(false)。
		/// </summary>
		[RequiresUniqueBuildEnvironment]
		...
		/// 核心UObject项目，禁用仅当构建独立应用，它不需要连接到内核。
		/// </summary>
		[RequiresUniqueBuildEnvironment]
		...
		/// 启用为构建 需要初始化应用的核心模块，命令行单元通常不需要它
		/// </summary>
		[RequiresUniqueBuildEnvironment]
		...
		/// 是否为该模块使用向后兼容的默认值。默认情况下，引擎模块总是使用最新的默认设置，而项目模块则不是(以支持更容易的迁移路径)。
		/// </summary>
		...
		/// 默认情况下，为此目标中的模块启用“包含您使用的内容”。将此项目中任何模块的默认PCH模式更改为pchusagmode . useexplicitorsharedpchs。
		/// </summary>
		[CommandLine("-IWYU")]
		...
		/// 指定启动模块的名称。对于模块化构建，这是编译到目标可执行文件中的模块。
		/// </summary>
		...
		/// 生成项目文件时，指定当有多个相同类型的目标时要使用的项目文件的名称。
		/// </summary>
